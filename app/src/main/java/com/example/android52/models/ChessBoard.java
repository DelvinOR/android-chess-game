...
import com.example.android52.MainActivity;
...
public class ChessBoard {
     ...
     */
    public boolean input(String move, int side) {
    ...
    public void movePiece(char currFile, int currRank, char nextFile, int nextRank) {
        ...
        board.get(nextFile)[nextRank - 1].setLocation(nextFile, nextRank);
        resetAllPossibleMoves();
    }
    ...
    public void resetAllPossibleMoves() {
        for (char f = 'a'; f <= 'h'; f++) {
            for (int r = 1; r <= 8; r++) {
                ChessPiece piece = board.get(f)[r-1];
                if(piece != null){
                    piece.resetPossibleMoves(board);
                }
            }
        }
    }
